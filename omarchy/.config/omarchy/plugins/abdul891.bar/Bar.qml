...
Item {
  ...
  // Plain properties with defaults, not `required` as in the built-in bar: a
  // plugin bar is instantiated by a Loader and only gets these afterwards,
  // from shell.qml's configureBar(), so a required property here fails the
  // load outright ("Required property barConfig was not initialized").
  ...
  // The only change this clone makes to the built-in bar: which outputs it
  // shows on. Names come from `bar.monitors` in shell.json (Hyprland output
  // names, e.g. "DP-2"); an empty or absent list means every screen, exactly
  // like the built-in bar. Panels for the other outputs stay mapped but
  // invisible and claim no exclusion zone, so nothing reserves space there.
  readonly property var enabledOutputs: Util.isPlainObject(barConfig) && Array.isArray(barConfig.monitors) ? barConfig.monitors : []
  function outputEnabled(screen) {
    if (enabledOutputs.length === 0) return true
    return enabledOutputs.indexOf(String(screen ? screen.name : "")) !== -1
  }
  ...
  component BarPanel: PanelWindow {
    ...
    readonly property bool outputEnabled: root.outputEnabled(modelData)
    ...
    visible: outputEnabled && !remapGuard.remapping
    exclusionMode: !outputEnabled || root.barHidden ? ExclusionMode.Ignore : ExclusionMode.Auto
