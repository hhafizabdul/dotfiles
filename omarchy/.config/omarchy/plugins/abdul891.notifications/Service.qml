...
Item {
  ...
  // -------------------------------------------------------------- popup UI
  //
  // One PanelWindow per output (Variants on Quickshell.screens), with the
  // notification stack visible only on DP-2. Layer is Overlay, exclusionMode
  // Ignore, no keyboard focus — popups are passive surfaces and must never
  // steal input from the focused application.
  ...
  Variants {
    ...
    PanelWindow {
      ...
      screen: modelData
      visible: modelData.name === "DP-2" && popupModel.count > 0
